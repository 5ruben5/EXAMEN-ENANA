...
class EnanaTest extends TestCase {
    ...
    public function testCreandoEnana() {
        ...
        $enana1 = new Enana("Ruben", 30); // Enana viva
        $this->assertEquals("Ruben", $enana1->getNombre());
        $this->assertEquals(30, $enana1->getPuntosVida());
        $this->assertEquals("viva", $enana1->getSituacion());

        $enana2 = new Enana("Nacho", -20); // Enana muerta
        $this->assertEquals("Nacho", $enana2->getNombre());
        $this->assertEquals(-20, $enana2->getPuntosVida());
        $this->assertEquals("muerta", $enana2->getSituacion());

        $enana3 = new Enana("Maria", 0); // Enana en limbo
        $this->assertEquals("Maria", $enana3->getNombre());
